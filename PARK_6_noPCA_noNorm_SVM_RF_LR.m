%% Classification PD and HC based on individual natural frequency maps for Parkinson's study (Arana et al., 2026)- no PCA and no Normalization
% This script is employed for internal cross validation of the model under
% the NO PCA with NO normalization scheme.
...
fprintf('Evaluating without PCA nor normalization...\n');
...
for rep = 1:n_reps
    ...
    for fold = 1:kfold
        ...
        % SVM
        model_linear = fitcsvm(X_train_use, Y_train, 'KernelFunction', 'linear', 'BoxConstraint', 1, 'Standardize', false, 'ClassNames', unique(Y));
        ...
        model_rf = TreeBagger(100, X_train_use, Y_train_rf, 'Method', 'classification', 'OOBPrediction', 'off', 'ClassNames', categories(Y_train_rf));
        ...
        % LR
        model_fl = fitclinear(X_train_use, Y_train, 'Learner', 'logistic', 'Regularization', 'ridge', 'ClassNames', unique(Y));
...
figure('Name','ROC no PCA','Position',[100 100 500 500]);
hold on;
...
% Accuracy violins
figure('Name','Accuracy (%) no PCA','Position',[100 100 500 500]);
...
fprintf('SVM: Sens = %.2f%% (%.2f - %.2f), Spec = %.2f%% (%.2f - %.2f)\n', mean(sens_svm_folds(:))*100, sens_svm_IC(1)*100, sens_svm_IC(2)*100, mean(spec_svm_folds(:))*100, spec_svm_IC(1)*100, spec_svm_IC(2)*100);
fprintf('RF: Sens = %.2f%% (%.2f - %.2f), Spec = %.2f%% (%.2f - %.2f)\n', mean(sens_rf_folds(:))*100, sens_rf_IC(1)*100, sens_rf_IC(2)*100, mean(spec_rf_folds(:))*100, spec_rf_IC(1)*100, spec_rf_IC(2)*100);
fprintf('LR: Sens = %.2f%% (%.2f - %.2f), Spec = %.2f%% (%.2f - %.2f)\n', mean(sens_fl_folds(:))*100, sens_fl_IC(1)*100, sens_fl_IC(2)*100, mean(spec_fl_folds(:))*100, spec_fl_IC(1)*100, spec_fl_IC(2)*100);
